        ...
        <div className="hr-side">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .6, delay: .2 }}>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', marginTop: '1rem', fontFamily: "'Poppins', sans-serif", fontSize: '.78rem', fontWeight: 600, color: T.green, textDecoration: 'none' }}
            >
        ...
        >
          ...
          <div style={{ position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              background: 'rgba(234, 128, 37,.18)',
              backdropFilter: 'blur(10px)',
              border: '1px solid rgba(234, 128, 37,.4)',
            ...
            <div style={{
              ...
              fontFamily: "'Poppins', sans-serif", fontSize: '.65rem', fontWeight: 600,
          ...
          <div style={{
            ...
            >
              <span style={{
                ...
                background: 'rgba(234, 128, 37,.1)', border: '1px solid rgba(234, 128, 37,.25)',
                fontFamily: "'Poppins', sans-serif", fontSize: '.65rem', fontWeight: 700,
                color: '#ea8025', letterSpacing: '.06em',
              }}>
                {post.category}
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '.3rem', fontSize: '.72rem', color: T.textMuted, fontFamily: "'Poppins', sans-serif" }}>
            ...
            {post.tags && (
              ...
              >
                {post.tags.slice(0, 3).map(tag => (
                  <span key={tag} style={{ padding: '.18rem .65rem', borderRadius: 100, background: 'rgba(234, 128, 37,.06)', border: `1px solid ${T.border}`, fontFamily: "'Poppins', sans-serif", fontSize: '.62rem', fontWeight: 600, color: T.textMuted }}>
                    #{tag}
                  </span>
                ))}
              </motion.div>
...
// ═══════════════════════════════════════════════════════════════
const BLOG_NODES = [
  { id: 'blog-n-0', Icon: TrendingUp,  label: 'Stratégie',  color: '#ea8025' },
  { id: 'blog-n-1', Icon: ShoppingCart,label: 'E-Commerce', color: '#ea580c' },
  { id: 'blog-n-2', Icon: Code,        label: 'Dev Web',    color: '#ea8025' },
  { id: 'blog-n-3', Icon: Globe,       label: 'SEO',        color: '#86efac' },
  { id: 'blog-n-4', Icon: Zap,         label: 'Growth',     color: '#ea580c' },
  { id: 'blog-n-5', Icon: BookOpen,    label: 'Ressources', color: '#ea8025' },
]
    ...
    <section ref={ref} style={{ padding: '3rem 5%', background: T.bgAlt, borderTop: `1px solid ${T.border}`, borderBottom: `1px solid ${T.border}` }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: '.65rem', fontWeight: 700, color: T.textMuted, textTransform: 'uppercase', letterSpacing: '.1em', flexShrink: 0 }}>Sujets</span>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.5rem' }}>
            {HASHTAGS.map((tag, i) => (
              <motion.span
                ...
                whileHover={{ y: -2, background: 'rgba(234, 128, 37,.14)', borderColor: 'rgba(234, 128, 37,.4)', color: '#ea8025' }}
                style={{ padding: '.28rem .75rem', borderRadius: 100, background: 'rgba(234, 128, 37,.06)', border: `1px solid ${T.border}`, fontFamily: "'Poppins', sans-serif", fontSize: '.7rem', fontWeight: 600, color: T.textSub, cursor: 'pointer', transition: 'all .2s' }}
              >
                {tag}
              </motion.span>
            ))}
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <BlurReveal delay={0.12}>
            <h2 className="section-title-big" style={{ position: 'relative', textAlign: 'center', fontSize: 'clamp(3.4rem,6.5vw,5.6rem)', fontWeight: 900, fontStyle: 'italic', fontFamily: "'Poppins', sans-serif", color: T.textMain, letterSpacing: '-.03em', lineHeight: 1.2 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr) auto repeat(3,1fr)', alignItems: 'center', gap: '1rem', position: 'relative', zIndex: 2 }}>
              ...
              {BLOG_NODES.slice(0, 3).map(({ id, Icon, label }) => (
                <div key={id} id={id}
                  ...
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: 'rgba(234, 128, 37,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon size={18} style={{ color: T.green }} />
                  </div>
                  <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: '.58rem', fontWeight: 600, color: T.textMuted, whiteSpace: 'nowrap' }}>{label}</span>
                </div>
              ))}
              {/* Center */}
              <div id="blog-center"
        ...
        <BlurReveal delay={0.08}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', marginBottom: '2rem', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap' }}>
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  style={{ padding: '.45rem 1.1rem', borderRadius: 100, border: '1px solid', cursor: 'pointer', fontFamily: "'Poppins', sans-serif", fontSize: '.8rem', fontWeight: 600, transition: 'all .2s', borderColor: activeCategory === cat ? T.green : T.border, background: activeCategory === cat ? '#ea8025' : 'transparent', color: activeCategory === cat ? '#fff' : T.textSub }}>
                  {cat}
                </button>
              ))}
            </div>
            <div style={{ position: 'relative' }}>
              ...
              <input
                style={{ padding: '.6rem 1rem .6rem 2.4rem', borderRadius: 100, border: `1px solid ${T.border}`, background: T.light ? '#f5f5f5' : 'rgba(234, 128, 37,.04)', color: T.textMain, fontFamily: "'Poppins', sans-serif", fontSize: '.82rem', outline: 'none', width: 220 }}
                placeholder="Rechercher..."
                value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                onFocus={e => e.target.style.borderColor = '#ea8025'}
                ...
                <BlurReveal key={post.slug} delay={i * 0.07} direction={['up','right','up','left','up','right'][i % 6]}>
                  <TiltCard className="sku-card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%' }}>
                    ...
                    <div style={{ height: 200, position: 'relative', overflow: 'hidden' }}>
                      ...
                      <div style={{ position: 'absolute', inset: 0, background: 'rgba(12, 10, 9, 0.6)' }} />
                    </div>
                    {/* Content */}
                    <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '.7rem', marginBottom: '.9rem' }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '.35rem', padding: '.22rem .75rem', borderRadius: 100, background: T.light ? 'rgba(234, 88, 12,.07)' : 'rgba(234, 128, 37,.07)', border: `1px solid ${T.border}`, fontFamily: "'Poppins', sans-serif", fontSize: '.65rem', fontWeight: 600, color: T.green, letterSpacing: '.06em' }}>
                      ...
                      <h3 style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: '.98rem', color: T.textMain, letterSpacing: '-.02em', lineHeight: 1.4, marginBottom: '.7rem' }}>
                        {post.title}
                      </h3>
                      ...
                      {post.linkedinUrl ? (
                        <a href={post.linkedinUrl}
                          target="_blank"
                          rel="noreferrer"
                          style={{ display: 'flex', alignItems: 'center', gap: '.4rem', fontFamily: "'Poppins', sans-serif", fontSize: '.65rem', fontWeight: 600, color: T.green, textDecoration: 'none', transition: 'gap .2s' }}
...
function Newsletter() {
  ...
  return (
    <section ref={sectionRef} style={{ padding: '5rem 5%', background: T.bgAlt }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <BlurReveal>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'rgba(234, 128, 37,.12)', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
            <Zap size={24} style={{ color: T.green }} />
          </div>
          <h2 className="section-title-big" style={{ position: 'relative', textAlign: 'center', fontSize: 'clamp(3.4rem,6.5vw,5.6rem)', fontWeight: 900, fontStyle: 'italic', fontFamily: "'Poppins', sans-serif", color: T.textMain, letterSpacing: '-.03em', marginBottom: '.7rem', textAlign: 'left' }}>
        ...
        />
          {done ? (
            <motion.div initial={{ scale: .8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              style={{ padding: '1.2rem 2rem', borderRadius: 14, background: 'rgba(234, 128, 37,.08)', border: `1px solid ${T.border}`, color: T.green, fontFamily: "'Poppins', sans-serif", fontWeight: 700 }}>
              ✅ Merci ! Vous êtes abonné.
            </motion.div>
          ) : (
            <div style={{ display: 'flex', gap: '.8rem', maxWidth: 520, margin: '0 auto' }}>
              <input type="email" placeholder="votre@email.com" value={email} onChange={e => setEmail(e.target.value)}
                style={{ flex: 1, padding: '.85rem 1rem', borderRadius: 10, border: `1px solid ${T.border}`, background: T.light ? '#f5f5f5' : 'rgba(234, 128, 37,.04)', color: T.textMain, fontFamily: "'Poppins', sans-serif", fontSize: '.88rem', outline: 'none' }}
                onFocus={e => e.target.style.borderColor = '#ea8025'}
                onBlur={e => e.target.style.borderColor = T.border}
              />
    ...
    <section style={{ position: 'relative' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 50, textAlign: 'left', padding: '2.5rem 5% 1.5rem', background: steps[0].bg, borderBottom: `1px solid ${T.border}` }}>
        <BlurReveal delay={0.1}>
          <h2 className="section-title-big" style={{ position: 'relative', textAlign: 'center', fontSize: 'clamp(3.4rem,6.5vw,5.6rem)', fontWeight: 900, fontStyle: 'italic', fontFamily: "'Poppins', sans-serif", color: T.textMain, letterSpacing: '-.03em', margin: 0 }}>
          ...
          <BlurReveal delay={0.05}>
            <div style={{ maxWidth: 720, width: '100%', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '3rem', alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                <div style={{ width: 80, height: 80, borderRadius: 24, background: 'rgba(234, 128, 37,.12)', border: '1px solid rgba(234, 128, 37,.28)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon size={32} style={{ color: '#ea8025' }} />
                </div>
                <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: '3.5rem', fontWeight: 900, color: 'rgba(234, 128, 37,.12)', lineHeight: 1, letterSpacing: '-.04em' }}>{n}</span>
              </div>
              <div>
                <h3 style={{ fontSize: 'clamp(1.5rem,3vw,2.2rem)', fontWeight: 900, fontStyle: 'italic', fontFamily: "'Poppins', sans-serif", color: T.textMain, letterSpacing: '-.03em', marginBottom: '1rem' }}>
                  {title}
                </h3>
                ...
                <div style={{ marginTop: '1.5rem', display: 'flex', gap: '.4rem' }}>
                  {steps.map((_, si) => (
                    <div key={si} style={{ height: 3, borderRadius: 2, flex: si <= i ? 2 : 1, background: si <= i ? '#ea8025' : T.border, transition: 'all .4s' }} />
                  ))}
                </div>
              </div>
            </div>
