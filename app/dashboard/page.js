...
const STATUS_COLORS = {
  NEW: '#5b8def', QUALIFIED: '#ea8025', CONTACTED: '#e0a83e',
  CONVERTED: '#3ee08a', LOST: '#e05e5e', ACTIVE: '#ea8025', ENDED: 'rgba(255,255,255,.4)',
}
...
function UsageBar({ label, usage = {}, T }) {
  ...
  const approxLimit = usage?.approxLimit ?? 1
  const pct = Math.min((count / (approxLimit || 1)) * 100, 100)
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 10, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(.75rem, 4vw, 2rem)' }}>
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '1.2rem 1.4rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {conversation.lead && (
            <div style={{ background: 'rgba(234, 128, 37,.06)', border: `1px solid ${T.border2}`, borderRadius: 10, padding: '.8rem 1rem', marginBottom: 4 }}>
...
function normalizeStats(d) {
  return {
    ...
    activity: d?.activity ?? [],
    aiUsage: {
      gemini: { count: 0, approxLimit: 250, ...(d?.aiUsage?.gemini || {}) },
      groq: { count: 0, approxLimit: 14400, ...(d?.aiUsage?.groq || {}) },
    },
  }
}
