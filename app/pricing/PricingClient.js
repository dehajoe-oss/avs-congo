        ...
        <div className="guarantee-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem' }}>
          {GARANTIES.map(({ icon: Icon, title, desc }, i) => (
            <BlurReveal key={title} delay={i * .07}>
              <motion.div
                whileHover={{ y: -5, borderColor: 'rgba(180, 112, 39,.35)' }}
              ...
              >
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(180, 112, 39,.1)', border: '1px solid rgba(180, 112, 39,.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon size={20} style={{ color: '#b47027' }} />
                </div>
                <div>
              ...
              {TRUST_TEXT.split(' ').map((word, i) => (
                <span
                  ...
                  style={{
                    ...
                    opacity: 0.08,
                    filter: 'blur(9px)',
                    color: TRUST_GREEN.has(word) ? '#b47027' : 'inherit',
                  }}
                >
                  {word}
                </span>
          ...
          {FAQ_ITEMS.map(({ q, a }, i) => (
            <BlurReveal key={q} delay={i * 0.06} direction={i % 2 === 0 ? 'left' : 'right'}>
              <motion.div className="sku-card"
                whileHover={{ borderColor: 'rgba(180, 112, 39,.25)' }}
