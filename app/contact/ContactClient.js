...
const CHANNELS = [
  ...
  { id: 'cnt-n-1', icon: Mail,          label: 'Email',    val: 'agrovetoservicescongo@gmail.com', href: 'mailto:agrovetoservicescongo@gmail.com', color: '#c47b2d', desc: 'Réponse sous 24h' },
  { id: 'cnt-n-2', icon: Phone,         label: 'Téléphone',val: '+242 05 633 70 50 / +242 06 967 75 67', href: 'tel:+242069677567', color: '#c47b2d', desc: 'Bureau: Lun–Sam 8h–18h | Urgences 24/7' },
  { id: 'cnt-n-3', icon: MapPin,        label: 'Localisation', val: "Quartier Socoprise, Av. Nelson Mandela, Rue Bissoute, Pointe-Noire", href: null, color: '#c47b2d', desc: 'Déplacements possibles en exploitation' },
]

function ChannelIcon({ Icon, color, T }) {
  return (
    <div style={{ width: 42, height: 42, borderRadius: 12, background: 'rgba(196, 123, 45,.1)', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function ContactChannels() {
  ...
  const beamConnections = [
    ...
    { id: 'cnt-n-1', color: '#c47b2d', cx: 30,  cy: -20 },
    { id: 'cnt-n-2', color: '#c47b2d', cx: -30, cy: 20  },
    ...
    <section ref={sectionRef} style={{ padding: 'clamp(3rem,6vw,5rem) 5% clamp(2rem,4vw,3rem)', background: T.bgAlt }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr) auto repeat(2,1fr)', alignItems: 'center', gap: '1rem', position: 'relative', zIndex: 2 }}>
              ...
              {CHANNELS.slice(0, 2).map(({ id, icon: Icon, label, color }) => (
                <div key={id} id={id}
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(196, 123, 45,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <BlurReveal delay={0.5}>
          <div style={{ marginTop: '1.5rem', padding: '1rem 1.3rem', borderRadius: 14, background: 'rgba(196, 123, 45,.04)', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: '.6rem' }}>
              {[
                ...
                { Icon: Globe, href: 'https://agrovetoservices.cg', label: 'Site officiel', color: '#c47b2d' },
              ].map(({ Icon, href, label, color }) => (
                <a key={label} href={href} target="_blank" rel="noreferrer" title={label}
                  style={{ width: 36, height: 36, borderRadius: 10, background: T.light ? 'rgba(0,0,0,.05)' : 'rgba(196, 123, 45,.06)', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.textSub, transition: 'all .2s' }}
                  ...
                  onMouseLeave={e => { e.currentTarget.style.background = T.light ? 'rgba(0,0,0,.05)' : 'rgba(196, 123, 45,.06)'; e.currentTarget.style.borderColor = T.border; e.currentTarget.style.color = T.textSub; e.currentTarget.style.transform = 'none' }}>
                  <Icon size={15} />
                </a>
              ))}
            </div>
...
function ProjectFormContact() {
  ...
  const inputStyle = {
    ...
    padding: '.75rem .95rem',
    borderRadius: 10,
    background: T.light ? '#ffffff' : 'rgba(196, 123, 45,.04)',
            ...
            <AnimatePresence mode="wait">
              {sent ? (
                <motion.div key="success" initial={{ opacity: 0, scale: .92 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
                  <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'rgba(196, 123, 45,.15)', border: '2px solid rgba(196, 123, 45,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem' }}>
                    <CheckCircle size={36} style={{ color: '#c47b2d' }} />
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.4rem', marginBottom: '1.4rem' }}>
                    <div>
                      ...
                      <input style={inputStyle} placeholder="Ex: Jean Malonga" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                        onFocus={e => { e.target.style.borderColor = '#c47b2d'; e.target.style.boxShadow = '0 0 0 3px rgba(196, 123, 45,.12)' }}
