    ...
    <section style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', background: T.bg }}>
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', padding: '7rem 5% 4rem' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7, delay: .2 }}
          style={{ borderRadius: 18, overflow: 'hidden', border: '1px solid rgba(180, 112, 39,.25)', boxShadow: '0 8px 24px rgba(0,0,0,.3)', height: 280 }}>
          <LazyImg src={cld('/images/dr_poutya.jpeg')} alt="Dr POUTYA - Directrice Générale AVS Congo" style={{ width:'100%',height:'100%',objectFit:'cover',objectPosition:'center 20%' }}
            placeholder={<div style={{ height:'100%',background:'#1c1917',display:'flex',alignItems:'center',justifyContent:'center' }}><Users size={36} style={{color:'rgba(180, 112, 39,.3)'}}/></div>} />
        </motion.div>
      </div>
    </section>
  )
    ...
    <section ref={ref} style={{ padding: '4.5rem 5%', background: T.bg, position: 'relative' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', padding: '.25rem .75rem', borderRadius: 100, background: 'rgba(180, 112, 39,.1)', border: `1px solid ${T.border}`, fontFamily: "'Poppins', sans-serif", fontSize: '.64rem', fontWeight: 600, color: '#b47027', marginBottom: '.8rem', textTransform: 'uppercase', letterSpacing: '.06em' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          <div className="sku-card" style={{ padding: '1.5rem', borderRadius: 16, border: `1px solid ${T.border}`, background: T.card }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', padding: '.2rem .65rem', borderRadius: 100, background: 'rgba(180, 112, 39,.12)', color: '#b47027', fontFamily: "'Poppins', sans-serif", fontSize: '.7rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: '.8rem' }}>
  ...
  return (
    <section ref={ref} style={{ padding: '4.5rem 5%', background: T.bgAlt, position: 'relative', borderTop: `1px solid ${T.border}` }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}>
        ...
        {/* Carte Identité */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.2rem', borderRadius: 16, background: 'rgba(180, 112, 39,.06)', border: '1px solid rgba(180, 112, 39,.25)', marginBottom: '1.5rem' }}>
          <div style={{ width: 62, height: 62, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, border: '2px solid rgba(180, 112, 39,.5)', boxShadow: '0 0 14px rgba(180, 112, 39,.2)' }}>
            <LazyImg
              src={cld('/images/dr_poutya.jpeg')}
              alt="Dr POUTYA SAIZONOU Marie-Rose Edwige Rakié"
              style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 20%' }}
              placeholder={<div style={{ width: 62, height: 62, background: 'rgba(180, 112, 39,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#b47027', fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: '1.1rem' }}>E</div>}
            />
          </div>
          <div>
    ...
    <section ref={ref} style={{ padding: '4.5rem 5%', background: T.bg, position: 'relative', borderTop: `1px solid ${T.border}` }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}>
        <div style={{ textAlign: 'center', marginBottom: '1.8rem' }}>
          ...
          <p style={{ fontFamily: "'Poppins', sans-serif", fontSize: '.92rem', fontStyle: 'italic', fontWeight: 700, color: '#b47027' }}>
            « Bâtir un avenir agropastoral fort, durable et conforme aux standards mondiaux. »
          </p>
        </div>

        <div style={{ padding: '1.5rem', borderRadius: 18, border: `1px solid rgba(180, 112, 39,.25)`, background: T.card, position: 'relative' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem', fontSize: '.86rem', lineHeight: 1.75, color: T.textMain }}>
            ...
            <p style={{ margin: 0, fontFamily: "'Poppins', sans-serif", fontSize: '.95rem', fontWeight: 800, fontStyle: 'italic', color: '#b47027' }}>
          ...
          <div style={{ marginTop: '1.6rem', paddingTop: '1.2rem', borderTop: `1px solid ${T.border}` }}>
            ...
            <div style={{ fontFamily: "'Poppins', sans-serif", fontSize: '.64rem', color: '#b47027', fontWeight: 600 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem' }}>
        {VALUES.map(({ icon: Icon, title, desc }, i) => (
          <motion.div key={title} className="sku-card"
            ...
            <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(180, 112, 39,.1)', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1rem' }}>
    ...
    <section ref={ref} style={{ padding: '5rem 5%', background: T.bg }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.45rem' }}>
        {SKILLS.map((s, i) => (
          <motion.span key={s} initial={{ opacity: 0, y: 8 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: .2 + i * .04 }}
            style={{ padding: '.32rem .8rem', background: 'rgba(180, 112, 39,.07)', border: `1px solid ${T.border}`, borderRadius: 100, fontFamily: "'Poppins', sans-serif", fontSize: '.62rem', fontWeight: 600, color: T.green, letterSpacing: '.05em' }}>
            {s}
          </motion.span>
        ))}
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: .35 }}
        ...
        <LazyImg src={'https://images.unsplash.com/photo-1441122456239-401e92b73c65?auto=format&fit=crop&w=800&q=80'} alt="Clinique Vétérinaire AVS" style={{ width: '100%', height: '100%', objectFit: 'cover' }}
          placeholder={<div style={{ height: '100%', background: '#1c1917', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Award size={36} style={{ color: 'rgba(180, 112, 39,.3)' }} /></div>} />
      </motion.div>
    </section>
  )
}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap', marginBottom: '1.8rem' }}>
            {['Clinique 24/7', 'Intervention terrain', 'FCFA & Mobile Money'].map(b => (
              <span key={b} style={{ padding: '.28rem .75rem', borderRadius: 100, background: 'rgba(180, 112, 39,.08)', border: '1px solid rgba(180, 112, 39,.2)', fontFamily: "'Poppins', sans-serif", fontSize: '.65rem', fontWeight: 600, color: '#b47027' }}>{b}</span>
            ))}
          </div>
        </motion.div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.65rem' }}>
          {PAYS.map(({ code, name, note, primary }, i) => (
            <motion.div key={name}
              ...
              style={{ padding: '.85rem 1rem', borderRadius: 12, background: primary ? 'rgba(180, 112, 39, 0.08)' : (T.light ? 'rgba(0,0,0,.03)' : 'rgba(255,255,255,.03)'), border: `1px solid ${primary ? 'rgba(180, 112, 39,.35)' : T.border}`, display: 'flex', alignItems: 'center', gap: '.65rem' }}>
              <FlagBadge code={code} primary={primary} />
              <div>
                <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: '.75rem', color: primary ? '#b47027' : T.textMain }}>{name}</div>
