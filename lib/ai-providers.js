...
function isModelUnavailableError(err) {
  ...
  return (
    /"code"\s*:\s*(404|429|500|502|503|504)/.test(msg) ||
    /not found|no longer available/i.test(msg) ||
    /quota|rate limit|too many requests/i.test(msg) ||
    /high demand|unavailable|service unavailable|overloaded/i.test(msg) ||
    /invalid.*api.*key|unauthorized|authentication/i.test(msg) ||
    /fetch failed|network|socket|timeout|econnreset|etimedout/i.test(msg)
  )
}
